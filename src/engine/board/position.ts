...
export const DIRECTION_OFFSETS: Record<Direction, { row: number; column: number }> = {
  n: { row: -1, column: 0 },
  s: { row: 1, column: 0 },
  e: { row: 0, column: 1 },
  w: { row: 0, column: -1 },
...
export function positionKey(position: Position): OccupancyKey {
  return positionKeyFromCoordinates(position.row, position.column);
}

export function positionKeyFromCoordinates(row: number, column: number): OccupancyKey {
  return row * BOARD_COLUMNS + column;
}

export function isInsideBoard(position: Position): boolean {
  return isInsideCoordinates(position.row, position.column);
}

export function isInsideCoordinates(row: number, column: number): boolean {
  return (
    row >= 0 &&
    row < BOARD_ROWS &&
    column >= 0 &&
